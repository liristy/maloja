...
class MediaLibrary:
    def __init__(self, root, snapshot, interval=300):
        ...
        try:
            ...
            if saved["root"] == str(self.root) and saved["version"] == 1:
    ...
    def scan(self):
        ...
        for directory, dirs, filenames in os.walk(self.root, onerror=walk_error, followlinks=False):
            ...
            artist_picture = self.picture(files, ["artist", "folder"])
            ...
            for path in nfos:
                try:
                    ...
                    add("track", metadata, track_picture or album_picture)
                    bare_track = {k: metadata[k] for k in ("title", "artists")}
                    ...
                    if len(relative.parts) >= 2:
                        ...
                        if artist_folder not in artist_pictures:
                            ...
                            artist_pictures[artist_folder] = self.picture(artist_files, ["artist", "folder"])
                        for artist in set(metadata["artists"] + album["artists"]):
                            if normalize(artist) == normalize(artist_folder.name):
                                add("artist", artist, artist_pictures[artist_folder])
            ...
            if len(relative.parts) == 2 and not nfos:
                ...
                for path in files.values():
                    if path.suffix.lower() in AUDIO_EXTENSIONS:
                        ...
                        add("track", {"artists": track["artists"], "title": title}, cover)
        ...
        status = {"matched": len(entries), "ambiguous": len(candidates) - len(entries), "errors": errors}
        atomic_write(self.snapshot, json.dumps({"version": 1, "root": str(self.root),
                     "entries": entries, "records": records, "status": status,
                     "conflicts": conflicts}, ensure_ascii=False).encode())
        self.entries, self.records, self.status = entries, records, status
        self.ready = True
        logger.info("Media artwork index updated: %s", status)
        return status

    def lookup(self, kind, entity):
        path = self.entries.get(identity(kind, entity))
        # Do not fall back to title-only or to a different album edition.
